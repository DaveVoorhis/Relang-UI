...
public class SuggestionboxDialog extends FeedbackDialog {
	...
	public static void launch(Shell shell) {
		try {
			shell.getDisplay().syncExec(() -> (new SuggestionboxDialog(shell)).open());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected void create(Shell shlSuggestionBox) {
		shlSuggestionBox.setSize(700, 500);
		shlSuggestionBox.setLayout(new FormLayout());

		Composite panelIntro = new Composite(shlSuggestionBox, SWT.NONE);
		...
		Label lblStep1 = new Label(shlSuggestionBox, SWT.NONE);
		...
		textFeedback = new Text(shlSuggestionBox, SWT.BORDER | SWT.WRAP | SWT.V_SCROLL);
		...
		Label lblStep2 = new Label(shlSuggestionBox, SWT.NONE);
		...
		textEmailAddress = new Text(shlSuggestionBox, SWT.BORDER);
		FormData fd_textEmailAddress = new FormData();
		textEmailAddress.setLayoutData(fd_textEmailAddress);

		Label lblStep3 = new Label(shlSuggestionBox, SWT.NONE);
		...
		treeDetails = new Tree(shlSuggestionBox, SWT.BORDER | SWT.CHECK);
		...
		lblProgress = new Label(shlSuggestionBox, SWT.NONE);
		...
		progressBar = new ProgressBar(shlSuggestionBox, SWT.NONE);
		FormData fd_progressBar = new FormData();
		progressBar.setLayoutData(fd_progressBar);

		btnCancel = new Button(shlSuggestionBox, SWT.NONE);
		FormData fd_btnCancel = new FormData();
		btnCancel.setLayoutData(fd_btnCancel);
		btnCancel.setText("Cancel");
		btnCancel.addListener(SWT.Selection, e -> doCancel());

		btnSend = new Button(shlSuggestionBox, SWT.NONE);
		...
		phoneHome = new Feedback(btnSend, lblProgress, progressBar) {
		    public void completed(SendStatus sendStatus) {	
		    	SuggestionboxDialog.this.completed(sendStatus);
		    }
			public void quit() {
				SuggestionboxDialog.this.quit();
			}
		};
